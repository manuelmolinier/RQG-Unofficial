...
export class RunequestActorSheet extends RunequestBaseActorSheet {
  ...
  getData() {
    ...
    context.effects = ActiveEffectRunequest.prepareActiveEffectCategories(
      this.actor.effects
    );
  ...
  _prepareCharacterItems(sheetData) {
      ...
      // Append to skills.
        ...
        if (i.data.skillcategory != undefined) {
          ...
          if (i.data.name == "Dodge") {
            i.data.base =
              actorData.data.data.characteristics.dexterity.value * 2;
            this._prepareSkill(i);
            defense.push(i);
          }
          if (i.data.name == "Jump") {
            i.data.base =
              actorData.data.data.characteristics.dexterity.value * 3;
            this._prepareSkill(i);
          }
    ...
    actorData.data.data.attributes.hitpoints.value =
      actorData.data.data.attributes.hitpoints.max - totalwounds;
  ...
  activateListeners(html) {
    ...
    html.find(".meleeattack-roll").mousedown((event) => {
      ...
      let skillname = game.i18n.localize(
        RQG.weaponskills[attack.data.skillused]
      );
      ...
      let catmodifier =
        data.data.skillcategory[skill.data.skillcategory].modifier;
  ...
  static async loadCompendium(compendium, filter = (item) => true) {
    ...
    let compendiumData = await RunequestActorSheet.loadCompendiumData(
      compendium
    );
